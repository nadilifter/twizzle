...
async function main() {
  ...
  try {
    ...
    const orgSubdomains: string[] = configs.map((c: any) => c.subdomain).filter(Boolean);
    console.log(`  Found ${orgSubdomains.length} active org subdomain(s)`);

    const allSubdomains = [...FIXED_SUBDOMAINS, ...orgSubdomains];
    console.log(
      `  Total to check: ${allSubdomains.length} (${FIXED_SUBDOMAINS.length} fixed + ${orgSubdomains.length} orgs)\n`
    );
