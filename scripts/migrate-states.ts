  ...
  for (const org of orgs) {
    if (org.stateProvince && org.stateProvince.length > 2) {
      ...
      if (code) {
        ...
        console.warn(
          `Warning: Could not map state "${org.stateProvince}" for Organization ${org.name}`
        );
      }
    }
  }
  ...
  for (const addr of addresses) {
    if (addr.stateProvince && addr.stateProvince.length > 2) {
      ...
      if (code) {
        ...
        console.warn(
          `Warning: Could not map state "${addr.stateProvince}" for UserBillingAddress ${addr.id}`
        );
      }
    }
  }
  ...
  for (const fac of facilities) {
    if (fac.stateProvince && fac.stateProvince.length > 2) {
      ...
      if (code) {
        ...
        console.warn(
          `Warning: Could not map state "${fac.stateProvince}" for Facility ${fac.name}`
        );
      }
    }
  }
  ...
  for (const comp of comps) {
    if (comp.stateProvince && comp.stateProvince.length > 2) {
      ...
      if (code) {
        ...
        console.warn(
          `Warning: Could not map state "${comp.stateProvince}" for Competition ${comp.name}`
        );
      }
    }
  }
