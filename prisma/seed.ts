...
async function main() {
  ...
  // ============================================
  // London Western Track & Field Club Demo Site
  // ============================================
  
  // Create London Western organization
  const londonWesternOrg = await prisma.organization.upsert({
    where: { slug: "london-western" },
    update: {},
    create: {
      name: "London Western Track & Field Club",
      slug: "london-western",
    },
  });
  console.log("Created organization:", londonWesternOrg.name);

  // Add Andrew to London Western as admin
  await prisma.organizationMember.upsert({
    where: {
      organizationId_userId: {
  ...
  // Create WebsiteConfig for London Western
  await prisma.websiteConfig.upsert({
    where: { organizationId: londonWesternOrg.id },
    update: {},
    create: {
      organizationId: londonWesternOrg.id,
      subdomain: "londonwestern",
      primaryColor: "#4A1D91", // Deep purple
      secondaryColor: "#D4AF37", // Gold
      heroHeadline: "Join",
      heroSubheadline: "Ontario's premier track and field club. From junior development to elite national competition, we have programs for athletes of all ages and abilities.",
      heroText: null,
      showCalendar: true,
  ...
  console.log("Created London Western website config");

  // Create London Western programs
  const lwPrograms = await Promise.all([
    prisma.program.upsert({
      where: { id: "lw-jd-fundamentals" },
      update: {},
      create: {
        id: "lw-jd-fundamentals",
        name: "JD FUNdamentals",
        description: "Introduction to track and field for ages 5-8. Focus on fundamental movement skills through games and activities.",
        level: "Junior Development",
        status: "ACTIVE",
        organizationId: londonWesternOrg.id,
        membershipTiers: {
          create: [
    ...
    prisma.program.upsert({
      where: { id: "lw-jd-foundational" },
      update: {},
      create: {
        id: "lw-jd-foundational",
        name: "JD FUNdational Skills",
        description: "Building athletic foundations for ages 9-12. Development of running, jumping, and throwing techniques.",
        level: "Junior Development",
        status: "ACTIVE",
        organizationId: londonWesternOrg.id,
        membershipTiers: {
    ...
    prisma.program.upsert({
      where: { id: "lw-provincial-dev" },
      update: {},
      create: {
        id: "lw-provincial-dev",
        name: "Provincial Development",
        description: "Competitive training for ages 13+. Preparation for provincial-level competition in track and field events.",
        level: "Provincial",
        status: "ACTIVE",
        organizationId: londonWesternOrg.id,
        membershipTiers: {
    ...
    prisma.program.upsert({
      where: { id: "lw-national-dev" },
      update: {},
      create: {
        id: "lw-national-dev",
        name: "National Development",
        description: "Elite training program for high-performance athletes. Pathway to national team selection.",
        level: "National",
        status: "ACTIVE",
        organizationId: londonWesternOrg.id,
        membershipTiers: {
    ...
    prisma.program.upsert({
      ...
      create: {
        id: "lw-masters",
        name: "Masters",
        description: "Training program for athletes 35+. Stay competitive and fit with structured training.",
        level: "Masters",
        status: "ACTIVE",
        organizationId: londonWesternOrg.id,
        membershipTiers: {
    ...
    prisma.program.upsert({
      where: { id: "lw-coach-official" },
      update: {},
      create: {
        id: "lw-coach-official",
        name: "Coach / Official",
        description: "Membership for coaches and officials. Includes Athletics Ontario registration.",
        level: "Special",
        status: "ACTIVE",
        organizationId: londonWesternOrg.id,
        membershipTiers: {
  ...
  console.log("Created London Western programs:", lwPrograms.length);
