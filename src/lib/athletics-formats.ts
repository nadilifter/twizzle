...
export function timeSeedToMs(
  ...
  return (hours ?? 0) * 3600000 + (minutes ?? 0) * 60000 + (seconds ?? 0) * 1000 + (ms ?? 0);
}
...
export function formatSeedMarkForDisplay(entry: SeedMarkFields, resultType: ResultType): string {
  switch (resultType) {
    case "TIME":
      if (
...
export function hasSeedValue(entry: Partial<SeedMarkFields>, resultType: ResultType): boolean {
  switch (resultType) {
    case "TIME":
      return (
        entry.seedHours != null ||
