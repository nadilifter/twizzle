      ...
      <DialogContent className="sm:max-w-[600px] h-[80vh] flex flex-col">
        <DialogHeader>
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            <Badge variant={getStatusVariant(feature.status)}>{formatStatus(feature.status)}</Badge>
            {feature.targetDate && (
            ...
            {feature.statusChangedAt && (
              <Badge variant="outline" className="flex items-center gap-1 text-muted-foreground">
                ...
                Updated{" "}
                {formatDistanceToNow(new Date(feature.statusChangedAt), { addSuffix: true })}
              </Badge>
            )}
          </div>
            ...
            <div className="space-y-4 shrink-0">
              ...
              {feature.author && (
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  ...
                  <span>
                    Submitted by{" "}
                    <span className="font-medium text-foreground">{feature.author.name}</span>
                  </span>
                </div>
              )}
            </div>
                        ...
                        <div className="flex items-center justify-between flex-wrap gap-2">
                          <div className="flex items-center gap-2">
                            ...
                            {(comment.isStaffReply || comment.author.isStaff) && (
                              <Badge
                                variant="secondary"
                                className="text-xs flex items-center gap-1 bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300"
                              >
