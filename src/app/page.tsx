...
export default async function Home() {
  ...
  // Check if we're on localhost:3000 (not twizzle.localhost)
  const isLocalhostOnly = host === "localhost:3000";

  if (session) {
    if (isLocalhostOnly) {
      // If on localhost:3000 with a session, redirect through bridge to ensure
      // proper session cookie is set on twizzle.localhost subdomains
      ...
      bridgeUrl.searchParams.set("callbackUrl", "http://admin.twizzle.localhost:3000/");
  ...
  if (isLocalhostOnly) {
    redirect("http://login.twizzle.localhost:3000");
  }
  redirect("/login");
}
