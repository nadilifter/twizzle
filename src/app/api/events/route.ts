...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!permissions.includes("*") && !permissions.includes("events.create")) {
    ...
    if (
      validatedData.requiredMembershipInstanceIds &&
      validatedData.requiredMembershipInstanceIds.length > 0
    ) {
