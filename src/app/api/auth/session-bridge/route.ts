 ...
 * but our app runs on *.twizzle.localhost subdomains, we need to:
 *
 * 1. Complete OAuth/credentials login on localhost:3000
 * 2. Pass a signed bridge token to this endpoint on twizzle.localhost
 * 3. This endpoint verifies the token and sets the session cookie for .twizzle.localhost
 *
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (currentEnv === "local") {
      ...
      cookieDomain = ".twizzle.localhost";
    } else {
