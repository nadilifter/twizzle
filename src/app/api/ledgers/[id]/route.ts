...
export async function GET(
  ...
  try {
    ...
    const glCode = await db.gLCode.findFirst({
      ...
      include: {
        programs: { select: { id: true, name: true, status: true, basePrice: true }, orderBy: { name: "asc" } },
        events: { select: { id: true, title: true, date: true, type: true }, orderBy: { date: "desc" }, take: 50 },
        competitions: { select: { id: true, name: true, status: true, startDate: true }, orderBy: { startDate: "desc" } },
        products: { select: { id: true, name: true, price: true, isActive: true }, orderBy: { name: "asc" } },
        membershipGroups: { select: { id: true, name: true, defaultPrice: true }, orderBy: { name: "asc" } },
        passes: { select: { id: true, name: true, price: true, status: true }, orderBy: { name: "asc" } },
        _count: {
          select: {
            programs: true,
            events: true,
...
export async function PATCH(
  ...
  try {
    ...
    if (validatedData.code) {
      ...
      if (existing) {
        return NextResponse.json({ error: "A GL code with this code already exists" }, { status: 409 })
      }
    }
