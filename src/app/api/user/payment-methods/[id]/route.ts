  ...
  try {
    ...
    if (body.isDefault === true) {
      ...
      const updated = await db.paymentMethod.update({
        where: { id: params.id },
        data: { isDefault: true },
        select: { id: true, type: true, brand: true, last4: true, expiry: true, isDefault: true, createdAt: true },
      })
    ...
    console.error("Update user payment method error:", error instanceof Error ? error.message : error)
  ...
  try {
    ...
    if (existing.adyenTokenId && existing.shopperReference) {
      try {
        ...
        console.error("Failed to disable payment method in Adyen (proceeding with local delete):", adyenError instanceof Error ? adyenError.message : adyenError)
      }
    }
    ...
    console.error("Delete user payment method error:", error instanceof Error ? error.message : error)
