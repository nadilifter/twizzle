...
export async function PATCH(
  ...
  try {
    ...
    if (existing.isAutoGenerated && !existing.group.isRecurring) {
      if (validatedData.startDate || validatedData.endDate || validatedData.billingInterval) {
        return NextResponse.json(
          { error: "Cannot modify dates or billing interval of auto-generated non-recurring instances. Update the group instead." },
          { status: 400 }
        );
      }
    }
    ...
    if (validatedData.autoRenewDate !== undefined) updateData.autoRenewDate = validatedData.autoRenewDate;
    if (validatedData.purchaseStartDate !== undefined) updateData.purchaseStartDate = validatedData.purchaseStartDate;
...
export async function DELETE(
  ...
  try {
    ...
    if (instance.isAutoGenerated && !instance.group.isRecurring) {
      return NextResponse.json(
        { error: "Cannot delete the auto-generated instance of a non-recurring membership group. Delete the group instead." },
        { status: 400 }
      );
    }
