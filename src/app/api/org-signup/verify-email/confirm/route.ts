...
import { validateVerificationCode, createVerifiedToken } from "@/lib/mfa";
import { z } from "zod";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (verified) {
      const proofToken = createVerifiedToken(email, "SIGNUP_VERIFICATION");
      return NextResponse.json(
        { verified: true, proofToken },
        { headers: rateLimitHeaders(rateLimit) }
      );
    }

    return NextResponse.json({ verified: false }, { headers: rateLimitHeaders(rateLimit) });
  } catch (error) {
