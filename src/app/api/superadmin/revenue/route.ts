...
export async function GET(request: NextRequest) {
  try {
    ...
    let mrr = 0;
    const subscriptionsByPlan: Record<
      string,
    ...
    const arpu =
      activePayingCustomers > 0 ? (mrr + totalTransactionFeeRevenue) / activePayingCustomers : 0;
    ...
    const avgFeeRate =
      settledTransactions.length > 0
        ? totalTransactionFeeRevenue /
          settledTransactions.reduce((sum, t) => sum + Number(t.amount), 0)
        : 0;
...
function buildTimeSeriesData(
  ...
  const result: Map<string, { subscriptionRevenue: number; transactionFeeRevenue: number }> =
    new Map();
