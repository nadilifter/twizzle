...
export async function GET(request: NextRequest) {
  try {
    ...
    const orgSummaries = await Promise.all(
      orgIds.slice(0, 50).map((orgId) =>
        getSettlementSummary(orgId, startDate, endDate)
      )
    )
    ...
    const statusMap = new Map(
      platformAccounts.map((a) => [a.organizationId, a.onboardingStatus])
    )
