...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!config) {
      return NextResponse.json({ error: "Queue configuration not found" }, { status: 404 });
    }

    return NextResponse.json(config);
  } catch (error) {
    console.error("Error fetching queue config:", error);
    return NextResponse.json({ error: "Failed to fetch queue configuration" }, { status: 500 });
  }
}
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ error: "Failed to update queue configuration" }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to delete queue configuration" }, { status: 500 });
  }
}
