...
export async function POST(
  ...
  try {
    ...
    if (discount.status === "DRAFT") {
      return NextResponse.json({ valid: false, error: "This discount code is not yet active" }, { status: 400 })
    }

    if (validFrom > now) {
      return NextResponse.json({ valid: false, error: "This discount code is not yet valid" }, { status: 400 })
    }
