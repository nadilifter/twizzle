...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to fetch tax and fee settings" },
      { status: 500 }
    )
  }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    if (taxRate !== undefined && (typeof taxRate !== "number" || !isFinite(taxRate) || taxRate < 0 || taxRate > 1)) {
      return NextResponse.json({ error: "Tax rate must be a finite number between 0 and 1" }, { status: 400 })
    }

    if (taxPaidBy !== undefined && !VALID_FEE_PAYERS.includes(taxPaidBy)) {
      return NextResponse.json({ error: "taxPaidBy must be CUSTOMER or ORGANIZATION" }, { status: 400 })
    }

    if (processingFeePaidBy !== undefined && !VALID_FEE_PAYERS.includes(processingFeePaidBy)) {
      return NextResponse.json({ error: "processingFeePaidBy must be CUSTOMER or ORGANIZATION" }, { status: 400 })
    }
    ...
    return NextResponse.json(
      { error: "Failed to update tax and fee settings" },
      { status: 500 }
    )
  }
}
