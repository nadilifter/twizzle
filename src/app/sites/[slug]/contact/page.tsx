...
export default async function ContactPage({ params }: { params: { slug: string } }) {
  ...
  const addressParts = facility
    ...
    : [
        org.street,
        org.city,
        [org.stateProvince, org.postalCode].filter(Boolean).join(" "),
        org.country,
      ].filter(Boolean);
      ...
      <div className="grid md:grid-cols-2 gap-12">
        <div>
          ...
          <p className="text-muted-foreground mb-8 text-lg">
            Have questions about our programs or classes? We&apos;re here to help! Reach out to us
            ...
            {addressParts.length > 0 && (
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">{addressParts.join(", ")}</p>
                </div>
              </div>
            )}
          </div>
