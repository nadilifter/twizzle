...
export function QueueConfiguration({ config, onClose, onUpdate, onDelete }: QueueConfigurationProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    scheduledStart: config.scheduledStart ? new Date(config.scheduledStart).toISOString().slice(0, 16) : "",
    scheduledEnd: config.scheduledEnd ? new Date(config.scheduledEnd).toISOString().slice(0, 16) : "",
  })
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (formData.activationType === "THRESHOLD") {
        ...
        payload.scheduledStart = formData.scheduledStart ? new Date(formData.scheduledStart).toISOString() : null
        payload.scheduledEnd = formData.scheduledEnd ? new Date(formData.scheduledEnd).toISOString() : null
      }
          ...
          <div className="grid gap-4">
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) => setFormData({ ...formData, reservationMinutes: parseInt(e.target.value) || 10 })}
              />
              <p className="text-xs text-muted-foreground">
                How long a user has to complete their registration after being admitted from the queue.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid gap-4">
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) => setFormData({ ...formData, maxConcurrent: parseInt(e.target.value) || 50 })}
              />
              <p className="text-xs text-muted-foreground">
          ...
          <div className="grid gap-4">
            <div className="grid gap-2">
              ...
              <Select
                ...
                onValueChange={(value) => setFormData({ ...formData, activationType: value as any })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            {formData.activationType === "THRESHOLD" && (
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({ ...formData, activationThreshold: parseInt(e.target.value) || 100 })}
                />
                <p className="text-xs text-muted-foreground">
          ...
          <Card className="border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/20">
            <CardHeader className="pb-2">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-base text-amber-800 dark:text-amber-200">Queue Active</CardTitle>
              </div>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-amber-800 dark:text-amber-200">
                When enabled, users will be placed in a virtual waiting room before accessing registration. 
                They will see their position in line and estimated wait time.
              </p>
            </CardContent>
          </Card>
        )}
