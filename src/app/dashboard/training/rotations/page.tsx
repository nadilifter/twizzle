        ...
        {templates.map((template) => (
          <Card key={template.id}>
            <CardHeader>
              ...
              <CardDescription>Breakdown of time allocation per class.</CardDescription>
            </CardHeader>
            <CardContent>
              {/* Visual Bar */}
              ...
              <div className="flex flex-wrap gap-4 text-sm">
                {template.blocks.map((block, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <div
                      className={`w-3 h-3 rounded-full ${block.color} border border-slate-300`}
                    />
                    <span className="font-medium">{block.name}</span>
                    <span className="text-muted-foreground">({block.duration}m)</span>
                    {i < template.blocks.length - 1 && (
              ...
              <Sheet>
                ...
                <SheetContent className="sm:max-w-xl">
                  <SheetHeader>
                    ...
                    <SheetDescription>Modify the timeline for this class type.</SheetDescription>
                  </SheetHeader>
                  <div className="grid gap-4 py-4">
                    <div className="grid gap-2">
                      ...
                      {template.blocks.map((block, i) => (
                        <div key={i} className="flex gap-2 items-end">
                          ...
                          <div className="grid gap-1 w-32">
                            ...
                            <Select
                              defaultValue={
                                block.color.includes("chart-1")
                                  ? "purple"
                                  : block.color.includes("chart-2")
                                    ? "cyan"
                                    : block.color.includes("chart-3")
                                      ? "pink"
                                      : block.color.includes("chart-4")
                                        ? "teal"
                                        : block.color.includes("chart-5")
                                          ? "indigo"
                                          : "gray"
                              }
                            >
