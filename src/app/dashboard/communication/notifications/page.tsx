...
const TIMING_DEFAULTS: Record<
  string,
  { value: number; unit: TimingUnit; direction: TimingDirection }
> = {
...
const DEFAULT_TIMING = {
  value: 0,
  unit: "MINUTES" as TimingUnit,
  direction: "AFTER" as TimingDirection,
};
...
export default function NotificationsPage() {
  ...
  const [availableMembershipGroups, setAvailableMembershipGroups] = useState<
  ...
  const fetchRules = async () => {
    try {
      ...
      const res = await fetch(
        "/api/notifications/rules?includeInactive=true&ensureSystemRules=true"
      );
  ...
  const fetchMembershipGroups = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setAvailableMembershipGroups(
          (data.data || data || []).map((g: any) => ({ id: g.id, name: g.name }))
        );
      }
    } catch {}
  };
  ...
  const canProceedFromTemplate =
    !!templateBody.trim() && (actionType !== "EMAIL" || !!templateSubject.trim());
  ...
  const canProceedFromCurrentStep = () => {
    switch (currentStepId) {
      case "trigger":
        return canProceedFromTrigger;
      case "delivery":
        return canProceedFromDelivery;
      case "recipients":
        return canProceedFromRecipients;
      case "template":
        return canProceedFromTemplate;
      case "review":
        return true;
      default:
        return false;
  ...
  const groupedPlaceholders = placeholders.reduce<Record<string, PlaceholderDefinition[]>>(
    (acc, p) => {
    ...
    <div className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Manage your automated communication workflows. System rules (marked with lock icon)
                  ...
                  <TableRow key={rule.id}>
                    ...
                    <TableCell>
                      <Badge variant="secondary">
                        {
                          RECIPIENT_LABELS[
                            (rule.recipientConfig?.recipientType as RecipientType) || "GUARDIANS"
                          ]
                        }
                      </Badge>
                    </TableCell>
          ...
          <div className="flex flex-col gap-4 px-6 pt-4">
            ...
            <StepperNav>
              {ruleStepper.state.all.map((step, index) => {
                const ruleCurrentIndex = ruleStepper.state.all.findIndex(
                  (s) => s.id === currentStepId
                );
                const isSkipped = actionType === "ANNOUNCEMENT" && step.id === "recipients";
                const status = isSkipped
                  ? ("inactive" as const)
                  : getStepStatus(index, ruleCurrentIndex);
                return (
                    ...
                    <StepperItem status={status}>
                      <StepperIndicator
                        ...
                        onClick={() => {
                          if (isSkipped) return;
                          const effective = getEffectiveSteps();
                          const effectiveCurrentIdx = effective.indexOf(
                            currentStepId as (typeof effective)[number]
                          );
                          const effectiveTargetIdx = effective.indexOf(
                            step.id as (typeof effective)[number]
                          );
                ...
                <div className="grid gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(val) => {
                      ...
                      if (
                        !editingRule &&
                        (!name || name === `${TRIGGER_TYPE_LABELS[triggerType]} Notification`)
                      ) {
              ...
              <div className="overflow-y-auto max-h-[calc(90vh-280px)] px-1 space-y-4 py-2">
                <div className="grid gap-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      {Object.entries(RECIPIENT_LABELS)
                ...
                <div className="rounded-md bg-muted p-4">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {recipientType === "GUARDIANS" &&
                      "The guardians of the affected athlete(s) will receive this notification."}
                    {recipientType === "MEMBERSHIP_HOLDERS" &&
                      "Only guardians with athletes who hold specific memberships will receive this notification."}
                    {recipientType === "INTERNAL_USERS" &&
                      "Only staff members, coaches, and administrators will receive this notification."}
                    {recipientType === "CUSTOM" &&
                      "Custom filter to target specific subsets of recipients."}
                  </p>
                </div>
            ...
            {currentStepId === "template" && (
              <div className="overflow-y-auto max-h-[calc(90vh-280px)] px-1 space-y-4 py-2">
                ...
                <div className="grid gap-2">
                  ...
                  {actionType === "SMS" && (
                    <p
                      className={`text-xs ${templateBody.length > 160 ? "text-amber-600 font-medium" : "text-muted-foreground"}`}
                    >
                      {templateBody.length}/160 characters
                      {templateBody.length > 160 &&
                        ` (${Math.ceil(templateBody.length / 153)} SMS segments)`}
                    </p>
                  )}
                </div>
                  ...
                  {actionType !== "ANNOUNCEMENT" && (
                    <div className="p-4 grid grid-cols-[120px_1fr] gap-2">
                      ...
                      <span className="text-sm">
                        ...
                        {recipientType === "MEMBERSHIP_HOLDERS" &&
                          filterMembershipGroupIds.length > 0 && (
                            <span className="ml-2 text-xs text-muted-foreground">
                              ({filterMembershipGroupIds.length} group
                              {filterMembershipGroupIds.length !== 1 ? "s" : ""} selected)
                            </span>
                          )}
                      </span>
                    </div>
        ...
        <CardHeader className="cursor-pointer" onClick={() => setShowLogs(!showLogs)}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>View the history of sent notifications.</CardDescription>
            </div>
