...
export function BillingGracePeriodBanner({ scheduledDeactivationDate }: BillingGracePeriodBannerProps) {
      ...
      <div className="flex items-start gap-3 max-w-screen-xl mx-auto">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-amber-800 dark:text-amber-200">
            Payment required &mdash; {daysRemaining} {daysRemaining === 1 ? "day" : "days"} remaining
          </p>
          <p className="text-sm text-amber-700 dark:text-amber-300 mt-0.5">
            We were unable to process your subscription payment. Please update your payment method to avoid
            service interruption. Your site will be deactivated on{" "}
          ...
          {retryResult && (
            <p className={`text-sm mt-1 font-medium ${retryResult.success ? "text-green-700" : "text-red-700"}`}>
              {retryResult.message}
            </p>
          )}
        </div>
