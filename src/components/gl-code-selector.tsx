...
const REVENUE_ENTITY_TYPES = new Set<string>([
  "PROGRAM", "EVENT", "COMPETITION", "MEMBERSHIP", "PASS", "PRODUCT",
])
...
export function GLCodeSelector({ value, onChange, entityType, label = "GL Code", className }: GLCodeSelectorProps) {
  ...
  const filtered = filterByType
    ? glCodes.filter((c) => c.type === "REVENUE")
    : glCodes
