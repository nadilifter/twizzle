...
export function configToRRule(config: RecurrenceConfig): string {
  const rruleOptions: Partial<ConstructorParameters<typeof RRule>[0]> = {
    freq: config.frequency === "DAILY" ? RRule.DAILY 
        : config.frequency === "WEEKLY" ? RRule.WEEKLY 
        : RRule.MONTHLY,
...
export function parseRRule(rruleString: string, startDate: Date, endDate?: Date): RecurrenceConfig {
  try {
    const rule = RRule.fromString(rruleString.startsWith("RRULE:") ? rruleString : `RRULE:${rruleString}`)
    
    const frequency: Frequency = 
      rule.options.freq === RRule.DAILY ? "DAILY" :
      rule.options.freq === RRule.WEEKLY ? "WEEKLY" : "MONTHLY"
      ...
      {previewDates.length > 0 && (
        <div className="space-y-2">
          <Label className="flex items-center gap-2 text-sm font-medium">
            ...
            Upcoming dates ({previewDates.length > 10 ? "first 10" : previewDates.length} occurrences)
          </Label>
